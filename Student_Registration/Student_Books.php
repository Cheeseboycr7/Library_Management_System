...
try {
    ...
    $book_type_filter = isset($_GET['type']) ? $conn->real_escape_string($_GET['type']) : '';
    $sort_by = isset($_GET['sort']) ? $conn->real_escape_string($_GET['sort']) : 'title_asc';
    
    // Base SQL query
    $sql = "SELECT SQL_CALC_FOUND_ROWS b.ISBN_NO, b.Title, b.Quantity, b.cover_image, b.reference_only,
                   GROUP_CONCAT(DISTINCT CONCAT(a.Name, ' ', a.Surname) SEPARATOR ', ') AS authors,
                   s.Category, p.Name_of_Publisher, s.BookShelf_NO, p.Publication_Date
            FROM book b
    ...
    // Add book type filter if provided
    if (!empty($book_type_filter)) {
        if ($book_type_filter === 'reference') {
            $sql .= " AND b.reference_only = 1";
        } elseif ($book_type_filter === 'borrowable') {
            $sql .= " AND b.reference_only = 0 AND b.Quantity > 0";
        }
    } else {
        // Default to showing both types
        $sql .= " AND (b.reference_only = 1 OR (b.reference_only = 0 AND b.Quantity > 0))";
    }
    
    $sql .= " GROUP BY b.ISBN_NO, b.Title, b.Quantity, b.cover_image, b.reference_only, 
              s.Category, p.Name_of_Publisher, s.BookShelf_NO, p.Publication_Date";
    ...
    switch ($sort_by) {
        ...
        case 'author_desc':
            ...
            break;
        case 'date_newest':
            $sql .= " ORDER BY p.Publication_Date DESC";
            break;
        case 'date_oldest':
            $sql .= " ORDER BY p.Publication_Date ASC";
            break;
        default: // title_asc
...
<body>
    ...
    <div class="content">
                ...
                <form method="GET" class="row g-3">
                    ...
                    <div class="col-md-2">
                        <select class="form-select" name="type">
                            ...
                            <option value="borrowable" <?= $book_type_filter === 'borrowable' ? 'selected' : '' ?>>Borrowable</option>
                            <option value="reference" <?= $book_type_filter === 'reference' ? 'selected' : '' ?>>Reference Only</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-2">
                        <select class="form-select sort-dropdown" name="sort">
                            ...
                            <option value="date_newest" <?= $sort_by === 'date_newest' ? 'selected' : '' ?>>Newest First</option>
        ...
        <?php if (!empty($books)): ?>
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4" id="books-grid">
                <?php foreach ($books as $book): ?>
                    <div class="col">
                        <div class="card book-card h-100 <?= $book['reference_only'] ? 'reference-only' : '' ?>">
                            <div class="position-relative">
                                ...
                                <?php if ($book['reference_only']): ?>
                                    <span class="book-badge reference-badge">
                                        <i class="fas fa-bookmark me-1"></i> Reference Only
                                    </span>
                                <?php elseif ($book['Quantity'] > 0): ?>
                                    <span class="book-badge availability-badge">
                                        <i class="fas fa-check-circle me-1"></i> Available: <?= (int)$book['Quantity']; ?>
                                    </span>
                                <?php endif; ?>
                            </div>
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <p class="text-muted">
                    <?php if (!empty($search_query) || !empty($category_filter) || !empty($book_type_filter)): ?>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.sort-dropdown, select[name="category"], select[name="type"]').change(function() {
