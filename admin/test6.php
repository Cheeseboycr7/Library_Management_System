...
$bookExists = false;
$existingBookDetails = null;

// Function to check if book exists
function checkBookExists($isbn, $conn) {
    $stmt = $conn->prepare("SELECT b.*, p.Name_of_Publisher, s.BookShelf_NO 
                           FROM book b
    ...
    if ($result->num_rows > 0) {
        ...
        // Get authors
        $stmt = $conn->prepare("SELECT a.Name, a.Surname 
                               FROM author a
...
// Check for existing book before processing form
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check_book'])) {
    $isbn = validate_input($_POST['isbn']);
    $isbnValidation = validateISBN($isbn);
    
    if ($isbnValidation['valid']) {
        $existingBookDetails = checkBookExists($isbn, $conn);
        if ($existingBookDetails) {
            $bookExists = true;
        } else {
            $success = "No book found with this ISBN. You can proceed to add it.";
        }
    } else {
        $error = $isbnValidation['message'];
    }
}

// Handle form submissions
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$bookExists) {
    try {
        ...
        // [Previous code for other sections remains the same]
...
<body>
    <div class="container">
        ...
        <?php if($bookExists && $existingBookDetails): ?>
            <!-- Display existing book details -->
            <div class="book-details">
                <h3>Book Already Exists</h3>
                
                <div class="detail-row">
                    <div class="detail-label">ISBN:</div>
                    <div class="detail-value"><?= htmlspecialchars($existingBookDetails['ISBN_NO']) ?></div>
                </div>
                
                <div class="detail-row">
                    <div class="detail-label">Title:</div>
                    <div class="detail-value"><?= htmlspecialchars($existingBookDetails['Title']) ?></div>
                </div>
                
                <div class="detail-row">
                    <div class="detail-label">Edition:</div>
                    <div class="detail-value"><?= htmlspecialchars($existingBookDetails['Edition_Statement']) ?></div>
                </div>
                
                <div class="detail-row">
                    <div class="detail-label">Price:</div>
                    <div class="detail-value"><?= htmlspecialchars($existingBookDetails['Price']) ?></div>
                </div>
                
                <div class="detail-row">
                    <div class="detail-label">Quantity:</div>
                    <div class="detail-value"><?= htmlspecialchars($existingBookDetails['Quantity']) ?></div>
                </div>
                
                <div class="detail-row">
                    <div class="detail-label">Language:</div>
                    <div class="detail-value"><?= htmlspecialchars($existingBookDetails['Language']) ?></div>
                </div>
                
                <div class="detail-row">
                    <div class="detail-label">Publisher:</div>
                    <div class="detail-value"><?= htmlspecialchars($existingBookDetails['Name_of_Publisher'] ?? 'N/A') ?></div>
                </div>
                
                <div class="detail-row">
                    <div class="detail-label">Shelf Number:</div>
                    <div class="detail-value"><?= htmlspecialchars($existingBookDetails['BookShelf_NO'] ?? 'N/A') ?></div>
                </div>
                ...
                <div class="detail-row">
                    ...
                    <div class="detail-value">
                        <div class="authors-list">
                            <?php foreach($existingBookDetails['authors'] as $author): ?>
                ...
                <?php if($existingBookDetails['cover_image']): ?>
                    <div class="detail-row">
                        <div class="detail-label">Cover Image:</div>
                        <div class="detail-value">
                            <img src="<?= htmlspecialchars($existingBookDetails['cover_image']) ?>" alt="Book Cover" class="book-cover">
                        </div>
                    </div>
                <?php endif; ?>
                
                <div class="action-buttons">
                    <a href="?" class="btn-primary">Add New Book</a>
                    <a href="try.php" class="btn-secondary">Back to Dashboard</a>
                </div>
            </div>
        <?php else: ?>
            ...
            <div class="section <?= $currentSection == 1 ? 'active' : '' ?>">
                ...
                <form method="POST" enctype="multipart/form-data" id="bookForm">
                    ...
                    <div class="navigation-buttons">
                        <button type="submit" name="check_book" class="btn-secondary">Check Book</button>
                        <button type="submit" name="save_book" class="btn-primary" <?= isset($_POST['check_book']) && !$bookExists ? '' : 'disabled' ?>>Save & Continue →</button>
                    </div>
                    
                    <?php if(!$bookExists && isset($_POST['check_book'])): ?>
                        <!-- Show the rest of the book form fields -->
                        <div class="form-group">
                            <label>Book Title
...
<div class="section <?= $currentSection == 4 ? 'active' : '' ?>">
    ...
    <script>
        ...
        // Enable save button if ISBN is valid
        document.getElementById('isbn').addEventListener('blur', function() {
            const isbn = this.value.replace(/[-\s]/g, '');
            const saveButton = document.querySelector('button[name="save_book"]');
