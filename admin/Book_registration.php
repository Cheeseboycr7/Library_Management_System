...
// ... [previous code remains the same until the book details section]
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['save_book'])) {
            ...
            $reference_only = isset($_POST['reference_only']) ? 1 : 0; // New field
            ...
            $stmt = $conn->prepare("INSERT INTO book (ISBN_NO, Title, Edition_Statement, Price, Quantity, Language, cover_image, Publisher_ID, reference_only) 
                                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssdsssii", $isbn, $title, $edition, $price, $quantity, $language, $cover_image, $publisher_id, $reference_only);
            $stmt->execute();
        ...
        <div class="section <?= $currentSection == 1 ? 'active' : '' ?>">
            ...
            <form method="POST" enctype="multipart/form-data" id="bookForm">
                ...
                <div class="checkbox-group">
                    <input type="checkbox" id="reference_only" name="reference_only" value="1">
                    <label for="reference_only">Reference Only (Library Use)</label>
                    <div class="tooltip-icon" title="Check this for books that cannot be borrowed">i</div>
                </div>
                <div class="reference-note">
                    Reference books cannot be borrowed and must remain in the library.
                </div>
